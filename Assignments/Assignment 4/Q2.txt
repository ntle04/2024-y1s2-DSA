...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    if(HashSearch(*Q1Ptr, key)!=NULL) //check for duplicate
        return 0;
    if(Q1Ptr->hSize !=0)
    ...
    if(Q1Ptr->Table[index].head == NULL){ //desired index is free
        Q1Ptr->Table[index].head = newNode;
    }
    else{ //desired index is not free
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    if(pre == NULL){ //head node is key to be deleted

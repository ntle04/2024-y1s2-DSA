...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
    int i; //keep track of ll1 index
    if((ll1 == NULL) || (ll2 == NULL)) return; //check for empty list

    while((ll2->size) != 0){
        if (findNode(ll1, i) == NULL) //check for alternate positions in ll1
            return;
        //if cannot insert ll2 item, end
        if(insertNode(ll1, i+1, ll2->head->item) == -1)
            return;
        removeNode(ll2, 0); //remove ll2 item after inserting
        i+=2;
    }
}

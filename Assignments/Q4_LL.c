...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    if((ll == NULL) || (ll->head == NULL)) return; //check for empty list/head
    slow = ll-> head;
    fast = ll->head;

    //for single-item lists
    if(ll->size == 1){
        insertNode(resultFrontList, 0, slow->item);
        return;
    }

    /*fast pointer moves twice as fast as slow pointer, slow pointer will be
    at the middle when fast pointer reaches NULL. so when the fast pointer
    reaches the end, the function inserts items into resultBackList instead */
    while((fast != NULL)&&(fast->next != NULL)){
        insertNode(resultFrontList, resultFrontList->size, slow->item);
        slow = slow->next;
        fast = fast->next->next;
    }

    //account for odd-numbered lists
    if (ll->size % 2 != 0) {
        insertNode(resultFrontList, resultFrontList->size, slow->item);
        slow = slow->next;
    }
    ...
    while(slow != NULL){
        insertNode(resultBackList, resultBackList->size, slow->item);
        slow = slow->next;
    }
}

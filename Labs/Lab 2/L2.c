...
void removeUntil(Stack *s, int value){
/* pops all values off a stack of integers down to but not
including the first occurrence of the chosen value */
...
void recursiveReverse(Queue *q){
/* reverses the order of items stored in a
queue of integers*/
    int n;
...
//using stacks AND queues
    ...
    //put whole string in a stack and queue, stripping out spaces
    printf("%s\n", word);
    while(*word){
        if(*word != " "){
            push(&s, toupper(*word));
    ...
    // divide size by 2, ignore middle character in odd-length string
    i = (s.ll.size)/2;

    //pop first half of tripped string off the stack
    //compare with second half
    while(i>0){
        if(pop(&s)!=dequeue(&q)){
            printf("The strong is not a palindrome\n");
            return;
        }
        i--;
    }
...
/* using ONLY stacks
    ...
    //pop half of the chara and push into s2
    i = (s1.ll.size)/2;
    odd = (s1.ll.size)%2;
    while(i>0){
        push(&s2, pop(&s1));
    }
    //if string has odd chara, pop middle one
    if(odd)
        pop(&s1);
    //compare 2 stacks
...
int balanced(char *expression){
/*t determines if an expression comprised of the characters
()[]{} is balanced*/
    Stack s;
    s.ll.head = NULL;
    s.ll.tail = NULL;
